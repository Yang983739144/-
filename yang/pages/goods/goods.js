...
Page({
    ...
    onLoad: function(t) {
        ...
        if (n) {
            ...
            if (c) var _ = c.total, u = c.carGoods; else var _ = {
                total_price: 0,
                total_num: 0
            }, u = [];
            i.setData({
        ...
        var l = 0, g = t.user_id, m = decodeURIComponent(t.scene);
        if (void 0 != g) l = g; else if (void 0 != m) {
    ...
    onConfirm: function(t) {
        ...
        for (var c = a.data.carGoods, _ = JSON.parse(e.attr), u = _.length, s = 0; s < u; s++) {
            var l = _[s].attr_list;
            if (JSON.stringify(l) == JSON.stringify(i)) var g = _[s].price, m = _[s].num;
        }
        ...
        if (w > m) {
            ...
            if (q) {
                for (var b = q.length, F = [], s = 0; s < b; s++) for (var y = q[s].goods, G = y.length, N = 0; N < G; N++) F.push(y[N]);
    ...
    guigejian: function(t) {
        ...
        if (0 == l) g = parseFloat(n.price); else var g = parseFloat(l);
        ...
        v -= 1;
        var w = a.data.total;
        w.total_num -= 1, w.total_price = parseFloat(w.total_price), l = parseFloat(l), 
        w.total_price -= l, w.total_price = w.total_price.toFixed(2);
        ...
        if (D) {
            for (var S = D.length, q = [], e = 0; e < S; e++) for (var b = D[e].goods, F = b.length, p = 0; p < F; p++) q.push(b[p]);
    ...
    jia: function(t) {
        ...
        if ((l = d.length) <= 0) d.push(s); else {
            for (h = 0; h < l; h++) d[h].goods_id == o.id && (d[h].num += 1, d[h].goods_price = e.toFixed(2), 
            n = !1);
            n && d.push(s);
        }
        ...
        if (u) {
            for (var l = u.length, g = [], m = 0; m < l; m++) for (var f = u[m].goods, p = f.length, h = 0; h < p; h++) g.push(f[h]);
    ...
    jian: function() {
        ...
        if (c) {
            for (var e = c.length, _ = [], u = 0; u < e; u++) for (var l = c[u].goods, g = l.length, s = 0; s < g; s++) _.push(l[s]);
    ...
    tianjia: function(t) {
        ...
        if (i) {
            ...
            if (1 == m.length) {
                ...
                for (var p = l.length, h = 0; h < p; h++) l[h].num += 1;
                for (var v = (y = a.data.carGoods).length, w = [], _ = 0; _ < v; _++) if (o.index == _) for (var x = y[_].attr, k = x.length, d = 0; d < k; d++) {
                    var D = {
                        attr_id: x[d].attr_id,
            ...
            var y = a.data.carGoods, G = y[o.index].attr, N = [];
            ...
            y[o.index].num > J) return wx.showToast({
                title: "商品库存不足",
                image: "/images/icon-warning.png"
